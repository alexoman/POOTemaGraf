...
Arbore::Arbore(int cop_nr_orase = 0, int cop_nrstrazi = 0,int cop_radacina = 0, vector<pair<double, double>> cop_coordonate_orase = {}, vector<vector<pair<int, double>>> cop_oras_distanta = {}):
...
{
    cout<<"Un arbore poate avea orice nod ca radacina.\nCiteste nodul pe care il dorest: ";
    cin>>radacina;
    if(radacina>nr_orase||radacina<1)
        throw Exception("Radacina nu poate avea indicele mai mare decat nr de orase sau mai mic de 1");
}

double Arbore::dist(int orasA, int orasB)
{
    cout<<"S-a apelat distanta din calsa Arbore"<<"\n";
...
istream& operator>>(istream &in, Arbore &arbore)
{
    ...
    cout<<"Dati coordonatele fiecarui oras\n";
    double xOras, yOras;
