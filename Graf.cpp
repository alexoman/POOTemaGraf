...
Graf::Graf(int cop_nr_orase=0,vector<pair<double, double>> cop_coordonate_orase={}, vector<vector<pair<int, double>>> cop_oras_distanta={}):
...
ostream& operator<< (ostream &out, Graf &g)
{
    ...
    for(int i=1;i<=g.nr_orase;++i)
    {
        ...
        {
            out<<"Orasul "<<i<<" are un drum direct cu urmatoarele orase: ";
